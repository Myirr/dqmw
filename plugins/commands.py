...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        if not await pgDb.get_chat(message.chat.id):
            await db.add_chat(message.chat.id, message.chat.title)
        return 
    if not await pgDb.is_user_exist(message.from_user.id):
    ...
    files = files_[0]
    title = file.get('file_name') if file.get('file_name') else 'No Name'
    size = file.get('file_size') if file.get('file_size') else '0Byts'

...
mydb = pymongo.MongoClient(DATABASE_URI)[DATABASE_NAME]
...
async def add_connection(group_id, user_id):
    try:
        ...
        if query and group_id in [x["group_id"] for x in query["group_details"]]:
            return False
...
async def active_connection(user_id):
    ...
    return int(query['active_group']) if query and query['active_group'] else None
...
async def if_active(user_id, group_id):
    query = mycol.find_one({"_id": user_id}, {"_id": 0, "group_details": 0})
    return query and query['active_group'] == group_id
...
async def delete_connection(user_id, group_id):
    try:
        ...
        if query and len(query["group_details"]) > 0:
